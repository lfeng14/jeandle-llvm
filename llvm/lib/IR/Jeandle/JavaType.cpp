...
JavaType jeandle::typeIntersect(JavaType A, JavaType B) {
  ...
  if (A.isKnown() && B.isKnown()) {
    ...
    assert(CB && CB->IsSubtype && "VMCallbacks must be set");
    if (A.Klass == B.Klass) {
      // Both constraints have the same Klass bound. Exact=false means "not
      // proven exact", so either exact constraint is sufficient to make their
      // intersection exact.
      Result.Klass = A.Klass;
      Result.Exact = A.Exact || B.Exact;
...
/// An exact receiver-class guard generated by profile devirtualization.
struct ExactKlassGuard {
  uintptr_t Klass = 0;
...
/// `ActualKlass` is loaded directly from `QueryObj` by `jeandle.load_klass`.
static ExactKlassGuard traceExactKlassGuard(Value *Cond, Value *QueryObj) {
  ...
  if (LoadCB->getArgOperand(0)->stripPointerCastsAndAliases() != QueryObj)
    return {};
  return {extractKlassConstant(CB->getArgOperand(0))};
}
...
static JavaType sharpenFromDominators(Value *V, Instruction *Context,
  ...
  for (auto *Node = DT.getNode(ContextBB); Node; Node = Node->getIDom()) {
    ...
    if (Klass != 0) {
      ...
      JavaType GuardType;
      GuardType.Klass = Klass;
      GuardType.Exact = IsExact;
      Best = typeIntersect(Best, GuardType);
    }
    if (Exclusions) {
